...
from data.config import moderation_role_id
...
class RollvVoiceCog(commands.Cog):
    ...
    def restore_active_rolls(self):
        ...
        cursor.execute(
            "SELECT id, gift, strftime('%Y-%m-%d %H:%M:%S', end_time), channel_id FROM rollv WHERE end_time > ?",
            (datetime.utcnow().isoformat(),))
        active_rolls = cursor.fetchall()
        conn.close()

        for roll_id, gift, end_time_str, channel_id in active_rolls:
            ...
            if time_remaining > 0:
                ...
                if channel:
                    try:
                        asyncio.create_task(
                            self.end_rollv(roll_id, gift, time_remaining, channel))
                    except Exception as e:
                        print(f"[restore_active_rolls] Ошибка: {e}")
    ...
    async def rollv(self, inter: disnake.ApplicationCommandInteraction, duration: int, gift: str, channels: str):
        ...
        cursor.execute("INSERT INTO rollv (gift, end_time, channel_id) VALUES (?, ?, ?)",
                       (gift, end_time, inter.channel.id))
